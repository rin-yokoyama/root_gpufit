...
int main(int argc, char **argv)
{
    ...
    TFile ofile("scratch/gpu_output.root", "recreate");
    ...
    auto gpuFit = [&fitter, &tree, &index, &params, &state, &chi_square, &n_iterations]()
    {
        fitter.CallGpufit();
    ...
    std::cout << "Gpufit: nFit=" << kNFit << " nEvent=" << kNEvent << std::endl;
    std::cout << "time = " << std::setw(13) << static_cast<double>(dt_gpufit) << std::setw(3) << "ms" << std::endl;
